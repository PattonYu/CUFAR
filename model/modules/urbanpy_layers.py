...
class LocalConv(nn.Module):
    def __init__(self, width, block_size, in_chn, out_chn):
        ...
        self.weight = nn.Parameter(torch.randn(self.n_blk * self.n_blk, self.bs ** 2 * in_chn,
                                               self.bs ** 2 * out_chn))  # 32*32 x 4*4*2 x 4*4*1
...
def batch_kl(yhat, y, scale, mask):
    ...
    loss = torch.where(mask_ > 0, loss,
                       Tensor(np.zeros(1)))  # [bs, n_blk, blk_size], sum on blk_size, mean on bs and n_blk
    loss = loss.sum([1, 2])
