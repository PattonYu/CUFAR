...
class continual:
    ...
    def draw_batches(self, c_map, f_map, ext, t):
        if t > 1:
            buffer_c_map, buffer_f_map, buffer_ext = self.buffer.get_data(self.args.minibatch_size, t)

            exp_c_map = torch.cat([c_map, buffer_c_map], dim= 0)
            exp_f_map = torch.cat([f_map, buffer_f_map], dim= 0)
            replay_ext = torch.cat([ext, buffer_ext], dim= 0)

            buffer_f_map, buffer_c_map, mask, aug = apply_augment(
                                        exp_f_map, 
                                        exp_c_map,
                                        last_c= c_map, 
                                        last_f= f_map, flag=False)

            alpha = self.MMD.forward(exp_c_map.view(exp_c_map.size(0), -1).cpu(), 
                                        c_map.view(c_map.size(0), -1).cpu())     
            alpha = 2 - (2/(1+math.exp(-alpha*3)))

            return buffer_c_map, buffer_f_map, replay_ext, alpha, mask, aug
        else:
            return c_map, f_map, ext, 1, 0, 0

    def observe(self, c_maps, f_maps, ext, t):
        # Reservoir sampling memory update:
        self.buffer.add_data(c_maps, f_maps, ext, t)
        replay_c_maps, replay_f_maps, replay_ext, alpha, mask, aug = self.draw_batches(c_maps, f_maps, ext, t)
        ...
        pred_f_map = self.model(replay_c_maps, replay_ext)
        if aug == "cutout":
