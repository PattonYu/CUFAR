...
class UrbanPy(nn.Module): #use proposalnet   
    def __init__(self, scales, N, in_channels=1, out_channels=1, n_residual_blocks=(16, 16), base_channels=64, ext_dim=7, 
                        img_width=32, img_height=32, ext_flag=True, n_res=1, islocal=False, compress=2):
        ...
        for i in range(N):
            ...
            geo_dim = 0 # 0 if geo_feature is not available
            block_size = 2**(i+1)
    ...
    def forward(self, x, ext):
        ...
        for i in range(self.N):
            ...
            if self.ext_flag:
                feat_maps = torch.cat([feat_maps, ext_out], dim=1)
